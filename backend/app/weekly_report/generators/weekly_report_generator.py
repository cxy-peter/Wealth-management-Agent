...
def generate_weekly_report(report_date: str | None = None, filters: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    report_markdown = "\n".join(
        [
            ...
            f"- 周报日期: {summary['report_date']} [evidence_id={summary['evidence_ids'][0] if summary['evidence_ids'] else 'ev_weekly_missing'}]",
            f"- 覆盖产品数: {summary['product_count']}；总规模 {kpis['total_scale_bn']:.2f} 亿元，较上周 {kpis['scale_wow_bn']:.2f} 亿元，较上月 {kpis['scale_mom_bn']:.2f} 亿元。 [evidence_id={kpis['scale_evidence_ids'][0] if kpis['scale_evidence_ids'] else 'ev_scale_missing'}]",
            f"- 基准区间内占比 {kpis['benchmark_pass_rate'] * 100:.1f}%，低于基准下限产品数 {kpis['benchmark_failed_count']}。 [evidence_id={kpis['benchmark_evidence_ids'][0] if kpis['benchmark_evidence_ids'] else 'ev_benchmark_missing'}]",
            f"- 低分位产品数 {kpis['low_percentile_product_count']}，需关注产品数 {kpis['attention_product_count']}，用于周报风险提示和后续人工复核队列。 [evidence_id={summary['evidence_ids'][-1] if summary['evidence_ids'] else 'ev_weekly_missing'}]",
        ]
    )
    return {
        **summary,
